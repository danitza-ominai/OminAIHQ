# PZ-009A - Registro de evidencia y validacion de procedencia
...
Registro por claim/evidence/version y consultas deterministas de soporte y contradiccion.

Problema que resuelve: registro de evidencia y validacion de procedencia como tramo acotado del recorrido del contrato; no amplifica la iniciativa cliente ni declara el MVP completo.
...
- Contrato: `6.3`, `11.7`, `RF-010`, `RF-011`, `RNF-014`, `CT-005`, `CT-006`.
...
- [PZ-008A](FICHA-PZ-008A.md) aceptada expresamente.

Mapear motivos funcionales a estados nucleares existentes; no agregar EVIDENCIA_REQUERIDA como nuevo estado ni EVIDENCIA_NO_DISPONIBLE como noveno error nuclear.
...
|---|---|---|
| Crear | `app/evidence_registry.py` | Solo esta pieza; detenerse si ya existe |
| Crear | `tests/test_evidence_registry.py` | Solo esta pieza; detenerse si ya existe |
...
Entrada: Hallazgos, fuentes y artefactos autorizados emitidos por agentes.

Salida: Registro por claim/evidence/version y consultas deterministas de soporte y contradiccion.
...
1. Referencia valida significa fuente resoluble y apoyo pertinente, no mera coincidencia textual.
2. Conservar originales, huellas y extractos mediante interfaz de artefactos; versionar cambios de fuente y no sobrescribir.
3. Fuente inaccesible, contradictoria y fuera del periodo generan estados visibles; no elevar confianza automaticamente.
4. Fuente desaparecida antes de aprobacion detiene el flujo: PAUSADA/BLOQUEADA nuclear con motivo EVIDENCIA_REQUERIDA y mensaje EVIDENCIA_NO_DISPONIBLE.
5. Revalidar disponibilidad antes de aprobar/exportar; no basta con un chequeo al importar.

## 7. Criterios de aceptacion y pruebas obligatorias

- [ ] AC-01: Una fuente respalda varias afirmaciones mediante relaciones explicitas; referencias cruzadas rechazadas.
- [ ] AC-02: Fechas publicacion/consulta diferenciadas y dato obsoleto no se presenta actual.
- [ ] AC-03: Retirar original entre validacion y decision impide aprobacion sin efectos.
- [ ] AC-04: Contradicciones conservan ambas fuentes y pendiente de decision.
- [ ] AC-05: Tras aprobacion, original eliminado conserva VBP historico y marca verificabilidad incompleta.
...
- `python -B -m unittest discover -s tests -p test_evidence_registry.py -v`
...
```text
Actua como constructor inicial exclusivamente de PZ-009A: Registro de evidencia y validacion de procedencia.
Trabaja en OminAIHQ y lee completa PIEZAS-PENDIENTES/FICHA-PZ-009A.md, sus fuentes y PIEZAS-PENDIENTES/00-REGLAS-Y-DECISIONES.md.
...
Comprueba aceptacion de PZ-008A y todos los requisitos del apartado 3. No uses archivos futuros ausentes como si ya existieran.

Puedes crear exclusivamente: app/evidence_registry.py; tests/test_evidence_registry.py.
...
Archivos autorizados efectivamente - crear: app/evidence_registry.py; tests/test_evidence_registry.py
