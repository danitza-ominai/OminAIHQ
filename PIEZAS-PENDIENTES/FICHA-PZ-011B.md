# PZ-011B - Evaluacion adversarial y control de mejoras
...
Reporte reproducible con bloqueadores, cinco dimensiones, umbrales, evidencia y comparacion de versiones.

Problema que resuelve: evaluacion adversarial y control de mejoras como tramo acotado del recorrido del contrato; no amplifica la iniciativa cliente ni declara el MVP completo.
...
- Contrato: `11.8`, `CT-010`, `CT-011`, `CT-012`, `CT-013`, `PT-004`, `PT-006`.
...
- [PZ-011A](FICHA-PZ-011A.md) aceptada expresamente.

Autorizar ejecucion de holdout separado; si esta contaminado declarar invalido y pedir reemplazo, no ocultarlo.
...
|---|---|---|
| Crear | `app/evaluation_report.py` | Solo esta pieza; detenerse si ya existe |
| Crear | `tests/test_evaluation_adversarial.py` | Solo esta pieza; detenerse si ya existe |
| Modificar | `app/evaluation_harness.py` | Solo integracion/cambio descrito aqui; conservar API y regresiones ajenas |
...
Entrada: Harness, dictamen Governance y trayectoria; baseline versionada.

Salida: Reporte reproducible con bloqueadores, cinco dimensiones, umbrales, evidencia y comparacion de versiones.
...
1. Reutilizar vbp_validation como fuente de pesos/bloqueadores; no introducir otra escala.
2. Medir resultado y conducta autorizada; una respuesta acertada con herramienta prohibida no pasa.
3. Mejora manual y autorizada de instrucciones fuera de este encargo; no loop autoevolutivo ni publicacion automatica.
4. Si cambia juez/metrica/dataset, invalidar comparacion y repetir evaluacion; preservar historial.
5. Declarar cada P1 seleccionado/diferido segun matriz; no optimizar solo semejanza textual.

## 7. Criterios de aceptacion y pruebas obligatorias

- [ ] AC-01: Puntaje alto con cita falsa, violacion de permiso o falta de aprobacion resulta NO_PASA.
- [ ] AC-02: Fronteras 69/70/79/80 y sumatoria 30/25/20/15/10 exactas.
- [ ] AC-03: Detectar fuga de holdout, manipulacion de evaluador y bypass de restricciones.
- [ ] AC-04: Reejecucion sobre version congelada produce resultados explicables; incertidumbre LLM registrada.
- [ ] AC-05: Reporte contiene exitos/fallos/causas y diferencia pruebas offline de llamadas reales.
...
- `python -B -m unittest discover -s tests -p test_evaluation_adversarial.py -v`
...
```text
Actua como constructor inicial exclusivamente de PZ-011B: Evaluacion adversarial y control de mejoras.
Trabaja en OminAIHQ y lee completa PIEZAS-PENDIENTES/FICHA-PZ-011B.md, sus fuentes y PIEZAS-PENDIENTES/00-REGLAS-Y-DECISIONES.md.
...
Comprueba aceptacion de PZ-011A y todos los requisitos del apartado 3. No uses archivos futuros ausentes como si ya existieran.

Puedes crear exclusivamente: app/evaluation_report.py; tests/test_evaluation_adversarial.py.
Puedes modificar exclusivamente: app/evaluation_harness.py.
...
Archivos autorizados efectivamente - crear: app/evaluation_report.py; tests/test_evaluation_adversarial.py
Archivos autorizados efectivamente - modificar: app/evaluation_harness.py
