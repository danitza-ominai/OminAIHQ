# PZ-010B - Recuperacion sin duplicar ejecuciones
...
Reanudacion desde ultimo punto valido con limites, resultados y autorizaciones conservados.

Problema que resuelve: recuperacion sin duplicar ejecuciones como tramo acotado del recorrido del contrato; no amplifica la iniciativa cliente ni declara el MVP completo.
...
- Contrato: `11.3-11.5`, `RF-021`, `RF-022`, `RNF-005`, `RNF-006`, `CT-003`, `CT-008`.
...
- [PZ-010A](FICHA-PZ-010A.md) aceptada expresamente.

Aprobar mecanismo de interrupcion de pruebas y root temporal; prueba no debe matar procesos del usuario ni tocar datos reales.
...
|---|---|---|
| Crear | `app/recovery.py` | Solo esta pieza; detenerse si ya existe |
| Crear | `tests/test_recovery.py` | Solo esta pieza; detenerse si ya existe |
...
Entrada: Repositorio durable, checkpoints y ledger de operaciones confirmadas.

Salida: Reanudacion desde ultimo punto valido con limites, resultados y autorizaciones conservados.
...
1. No restablecer intentos, presupuesto o tiempo activo confirmado al reiniciar.
2. Estado INDETERMINADO de una llamada externa sin recibo no se reintenta automaticamente; pedir decision, no prometer exactly-once de proveedor.
3. Verificar integridad, version, evidencias frescas y autorizacion vigente antes de continuar.
4. Continuar tarea pendiente, no repetir completada; propietario unico/lease local para evitar dobleejecucion.
5. Recuperar aclaracion, tarea y exportacion usando transacciones/checkpoints; sin orquestadores event-driven.

## 7. Criterios de aceptacion y pruebas obligatorias

- [ ] AC-01: Interrumpir antes/despues de guardar cada hito y comparar conteos/versiones.
- [ ] AC-02: Recuperar en un proceso nuevo sobre datos de prueba, no solo recrear un objeto.
- [ ] AC-03: Checkpoint corrupto, evidencia ausente y aprobacion obsoleta bloquean.
- [ ] AC-04: Llamada confirmada no se repite; resultado incierto se muestra y pausa.
- [ ] AC-05: Agotar limite tras reinicio sin restaurar presupuesto.
...
- `python -B -m unittest discover -s tests -p test_recovery.py -v`
...
Perfil de efectos: **IO_LOCAL_Y_SUBPROCESOS_DE_PRUEBA_ACOTADOS**.
...
```text
Actua como constructor inicial exclusivamente de PZ-010B: Recuperacion sin duplicar ejecuciones.
Trabaja en OminAIHQ y lee completa PIEZAS-PENDIENTES/FICHA-PZ-010B.md, sus fuentes y PIEZAS-PENDIENTES/00-REGLAS-Y-DECISIONES.md.
...
Comprueba aceptacion de PZ-010A y todos los requisitos del apartado 3. No uses archivos futuros ausentes como si ya existieran.

Puedes crear exclusivamente: app/recovery.py; tests/test_recovery.py.
...
Respeta el perfil de efectos IO_LOCAL_Y_SUBPROCESOS_DE_PRUEBA_ACOTADOS; todo permiso de instalacion, datos, nube, gasto o publicacion debe estar aprobado de forma concreta.
...
Archivos autorizados efectivamente - crear: app/recovery.py; tests/test_recovery.py
