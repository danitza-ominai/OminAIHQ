# PZ-013C - VBP visible, descarga fiel y demo ES/EN
...
VBP canonico visible y descargable .md; contenido esencial ES/EN para expediente de demo.

Problema que resuelve: vbp visible, descarga fiel y demo es/en como tramo acotado del recorrido del contrato; no amplifica la iniciativa cliente ni declara el MVP completo.
...
- Contrato: `6.6`, `9.6`, `11.9`, `RF-018`, `RF-020`, `RF-027`, `RF-030`, `DN-005`, `DN-009`, `CT-016`, `CT-017`.
...
- [PZ-013B](FICHA-PZ-013B.md) aceptada expresamente.

Ratificar formato bilingual y dominio de huella respecto de traduccion; si requiere cambiar vbp_document.py, pedir nueva lista exacta antes de editar.
...
|---|---|---|
| Crear | `app/vbp_export.py` | Solo esta pieza; detenerse si ya existe |
| Crear | `web/i18n.js` | Solo esta pieza; detenerse si ya existe |
| Crear | `tests/test_vbp_export.py` | Solo esta pieza; detenerse si ya existe |
| Crear | `tests/test_bilingual_view.py` | Solo esta pieza; detenerse si ya existe |
| Modificar | `app/http_api.py` | Solo integracion/cambio descrito aqui; conservar API y regresiones ajenas |
| Modificar | `web/app.js` | Solo integracion/cambio descrito aqui; conservar API y regresiones ajenas |
| Modificar | `web/index.html` | Solo integracion/cambio descrito aqui; conservar API y regresiones ajenas |
...
Entrada: VBP congelado y aprobado, informe de Governance y version de traduccion revisada cuando aplique.

Salida: VBP canonico visible y descargable .md; contenido esencial ES/EN para expediente de demo.
...
1. No otro VBP JSON/PDF ni traduccion paralela como autoridad. Manifest, contenido y auditoria en el unico Markdown.
2. Traduccion importante se prepara/revisa antes de aprobacion y se vincula a version; no alterar significado despues de aprobar.
3. Demo: mostrar ES y bloque #english o selector de vista fiel, manteniendo IDs y metadatos originales; no traducir silenciosamente decisiones.
4. Comprobar aprobacion, huella, 18 secciones y disponibilidad de evidencia antes de entregar; solo entonces FINALIZADA.
5. Descarga idempotente y fallo recuperable; no duplicar fuentes privadas dentro del paquete.
6. README general se actualiza al cierre documental de PZ-015B, no modificarlo aqui por comodidad.

## 7. Criterios de aceptacion y pruebas obligatorias

- [ ] AC-01: Bytes/contenido exportado coinciden con lo aprobado; alteracion impide descarga/finalizacion.
- [ ] AC-02: Duplicado descarga mismo artefacto; fallo de escritura/respuesta deja estado recuperable.
- [ ] AC-03: Render seguro contra HTML/scripts y enlaces maliciosos.
- [ ] AC-04: ES/EN cubre mision, estados, decisiones, evidencia, errores, puntuacion y VBP; revision humana de fidelidad.
- [ ] AC-05: Paquete conserva etiquetas SIMULADA/PENDIENTE y advertencia de fuente eliminada posterior.
...
Perfil de efectos: **DESCARGA_LOCAL_AUTORIZADA_Y_PRUEBAS_VISUALES**.
...
```text
Actua como constructor inicial exclusivamente de PZ-013C: VBP visible, descarga fiel y demo ES/EN.
Trabaja en OminAIHQ y lee completa PIEZAS-PENDIENTES/FICHA-PZ-013C.md, sus fuentes y PIEZAS-PENDIENTES/00-REGLAS-Y-DECISIONES.md.
...
Comprueba aceptacion de PZ-013B y todos los requisitos del apartado 3. No uses archivos futuros ausentes como si ya existieran.

Puedes crear exclusivamente: app/vbp_export.py; web/i18n.js; tests/test_vbp_export.py; tests/test_bilingual_view.py.
Puedes modificar exclusivamente: app/http_api.py; web/app.js; web/index.html.
...
Respeta el perfil de efectos DESCARGA_LOCAL_AUTORIZADA_Y_PRUEBAS_VISUALES; todo permiso de instalacion, datos, nube, gasto o publicacion debe estar aprobado de forma concreta.
...
Archivos autorizados efectivamente - crear: app/vbp_export.py; web/i18n.js; tests/test_vbp_export.py; tests/test_bilingual_view.py
Archivos autorizados efectivamente - modificar: app/http_api.py; web/app.js; web/index.html
