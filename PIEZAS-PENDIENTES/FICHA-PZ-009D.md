# PZ-009D - Auditoria consultable y trazas minimizadas
...
Linea de tiempo consultable por mision/tarea/fuente/decision y proyeccion minimizada de auditoria.

Problema que resuelve: auditoria consultable y trazas minimizadas como tramo acotado del recorrido del contrato; no amplifica la iniciativa cliente ni declara el MVP completo.
...
- Contrato: `6.5`, `11.1`, `RF-019`, `RF-020`, `RNF-001`, `RNF-009`, `CT-014`.
...
- [PZ-009C](FICHA-PZ-009C.md) aceptada expresamente.

Ratificar campos visibles/retencion con la politica de datos, sin crear un log paralelo fuera del repositorio de mision.
...
|---|---|---|
| Crear | `app/audit_query.py` | Solo esta pieza; detenerse si ya existe |
| Crear | `tests/test_audit_query.py` | Solo esta pieza; detenerse si ya existe |
...
Entrada: Eventos nucleares, artefactos, errores, herramientas, decisiones y limites.

Salida: Linea de tiempo consultable por mision/tarea/fuente/decision y proyeccion minimizada de auditoria.
...
1. Relacionar eventos con registros reales; distinguir hora de hecho y consulta.
2. Registrar acciones observables, herramientas y resultados resumidos; no CoT ni prompts completos.
3. Filtrar por mision/usuario y nunca recorrer todo el disco o exponer ruta privada.
4. Auditoria historica no reescribe decisiones ni confirma evidencia ahora eliminada.
5. Proyeccion para UI/exportacion; almacenamiento durable se implementa en PZ-010A.

## 7. Criterios de aceptacion y pruebas obligatorias

- [ ] AC-01: Reconstruir recorrido con fuentes, aprobaciones, errores, intentos y checkpoints.
- [ ] AC-02: Referencias ausentes y eventos duplicados se detectan.
- [ ] AC-03: Filtros no filtran datos de otra mision; ausencia de secretos y CoT.
- [ ] AC-04: Comparar trazas antes/despues de un fallo sin modificar historia.
- [ ] AC-05: Auditoria saneada conserva utilidad sin revelar contenidos eliminados.
...
- `python -B -m unittest discover -s tests -p test_audit_query.py -v`
...
```text
Actua como constructor inicial exclusivamente de PZ-009D: Auditoria consultable y trazas minimizadas.
Trabaja en OminAIHQ y lee completa PIEZAS-PENDIENTES/FICHA-PZ-009D.md, sus fuentes y PIEZAS-PENDIENTES/00-REGLAS-Y-DECISIONES.md.
...
Comprueba aceptacion de PZ-009C y todos los requisitos del apartado 3. No uses archivos futuros ausentes como si ya existieran.

Puedes crear exclusivamente: app/audit_query.py; tests/test_audit_query.py.
...
Archivos autorizados efectivamente - crear: app/audit_query.py; tests/test_audit_query.py
