# PZ-007A - Delivery Planner y dependencias verificables
...
Fases y tareas trazadas a requisito/riesgo, con pruebas, dependencias y supuestos de estimacion.

Problema que resuelve: delivery planner y dependencias verificables como tramo acotado del recorrido del contrato; no amplifica la iniciativa cliente ni declara el MVP completo.
...
- Contrato: `5.8`, `RF-013`, `RF-024`, `CT-007`, `DN-001`.
...
- [PZ-006A](FICHA-PZ-006A.md) aceptada expresamente.

No prometer una fecha por disponer de las fichas. Aprobar datos/costo de ensayo real.
...
|---|---|---|
| Crear | `app/delivery_planner.py` | Solo esta pieza; detenerse si ya existe |
| Crear | `tests/test_delivery_planner.py` | Solo esta pieza; detenerse si ya existe |
...
Entrada: Definicion de producto, requisitos, riesgos y restricciones aprobadas.

Salida: Fases y tareas trazadas a requisito/riesgo, con pruebas, dependencias y supuestos de estimacion.
...
1. Secuencia finita y no recursiva; no autorizar construccion ni contratar recursos.
2. Cada tarea tiene criterio, prueba, dependencia y limite; no inventar fecha o disponibilidad como hecho.
3. Detectar ciclo, referencia ausente, recurso no confirmado y plazo incompatible.
4. Distinguir rondas de correccion del VBP de las rondas del equipo constructor.
5. Registrar exclusivamente este especialista en hq_runtime.

## 7. Criterios de aceptacion y pruebas obligatorias

- [ ] AC-01: Trazabilidad de todas las tareas a requisito o riesgo.
- [ ] AC-02: Ciclo, dependencia propia/ausente y tarea incompleta rechazados con detalle.
- [ ] AC-03: Estimacion no respaldada queda SUPUESTO y no compromiso.
- [ ] AC-04: Presupuestos/tiempos sin limite impiden el inicio.
- [ ] AC-05: Llamada real opt-in separada, sin paralelismo ni acciones externas.
...
- `python -B -m unittest discover -s tests -p test_delivery_planner.py -v`
...
```text
Actua como constructor inicial exclusivamente de PZ-007A: Delivery Planner y dependencias verificables.
Trabaja en OminAIHQ y lee completa PIEZAS-PENDIENTES/FICHA-PZ-007A.md, sus fuentes y PIEZAS-PENDIENTES/00-REGLAS-Y-DECISIONES.md.
...
Comprueba aceptacion de PZ-006A y todos los requisitos del apartado 3. No uses archivos futuros ausentes como si ya existieran.

Puedes crear exclusivamente: app/delivery_planner.py; tests/test_delivery_planner.py.
...
Archivos autorizados efectivamente - crear: app/delivery_planner.py; tests/test_delivery_planner.py
