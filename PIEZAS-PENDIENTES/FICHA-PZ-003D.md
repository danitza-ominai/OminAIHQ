# PZ-003D - Motor secuencial de tareas SIMULADA
...
Motor inyectable de secuencia fija; cuatro especialistas ficticios llegan solo a EN_CONSOLIDACION.

Problema que resuelve: motor secuencial de tareas simulada como tramo acotado del recorrido del contrato; no amplifica la iniciativa cliente ni declara el MVP completo.
...
- Contrato: `4`, `11.3-11.4`, `RF-007`, `RF-008`, `RF-023`, `RF-024`, `RF-026`, `RF-027`, `CT-007`.
...
- [PZ-003C](FICHA-PZ-003C.md) aceptada expresamente.

Ratificar interfaces del motor tras aceptar PZ-003C; esta aprobacion no autoriza llamadas reales a modelos.
...
|---|---|---|
| Crear | `app/mission_engine.py` | Solo esta pieza; detenerse si ya existe |
| Crear | `app/simulated_specialists.py` | Solo esta pieza; detenerse si ya existe |
| Crear | `tests/test_mission_engine.py` | Solo esta pieza; detenerse si ya existe |
...
Entrada: Instantanea validada de una PlanReviewSession del mismo proceso, tras decision local explicita; contratos de PZ-003C.

Salida: Motor inyectable de secuencia fija; cuatro especialistas ficticios llegan solo a EN_CONSOLIDACION.
...
1. Consumir copias, nunca editar el objeto interno de PZ-003B; no cargar stdout previo como sesion autenticada.
2. Comprobar aprobacion consumida APROBAR, propietario, version/huella y referencias antes de MT-006.
3. Motor determinista independiente de proveedores; adaptador SIMULADA separado y siempre visible. Una tarea activa, dependencias verificadas, sin recursividad.
4. Aplicar transiciones de tareas y mision del nucleo; contador por intento y politicas finitas de 11.3/11.4; sin VBP final.
5. Resultados PARTIAL/BLOCKED/FAILED conservan limitaciones y bloquean descendientes cuando corresponda; no convertir excepciones en exito.

## 7. Criterios de aceptacion y pruebas obligatorias

- [ ] AC-01: Sin aprobacion, version obsoleta, rol ajeno o referencia rota: cero tareas activas e intentos.
- [ ] AC-02: Ejecutar las cuatro tareas exactamente en secuencia; duplicados no repiten efectos.
- [ ] AC-03: Fallar cada tarea y comprobar bloqueo descendiente, checkpoint en memoria y error tipado.
- [ ] AC-04: Agotar intentos, tiempo y solicitudes con relojes controlados; no reiniciar contadores.
- [ ] AC-05: Interceptar sockets, modelos, subprocesos y archivos: cero efectos externos; etiquetas SIMULADA en todos los resultados.
...
- `python -B -m unittest discover -s tests -p test_mission_engine.py -v`
...
```text
Actua como constructor inicial exclusivamente de PZ-003D: Motor secuencial de tareas SIMULADA.
Trabaja en OminAIHQ y lee completa PIEZAS-PENDIENTES/FICHA-PZ-003D.md, sus fuentes y PIEZAS-PENDIENTES/00-REGLAS-Y-DECISIONES.md.
...
Comprueba aceptacion de PZ-003C y todos los requisitos del apartado 3. No uses archivos futuros ausentes como si ya existieran.

Puedes crear exclusivamente: app/mission_engine.py; app/simulated_specialists.py; tests/test_mission_engine.py.
...
Archivos autorizados efectivamente - crear: app/mission_engine.py; app/simulated_specialists.py; tests/test_mission_engine.py
