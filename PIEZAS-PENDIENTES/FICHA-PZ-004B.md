# PZ-004B - Chief of Staff y composicion del runtime
...
Chief real de aclaracion, propuesta de plan y consolidacion; punto de composicion para el resto del runtime.

Problema que resuelve: chief of staff y composicion del runtime como tramo acotado del recorrido del contrato; no amplifica la iniciativa cliente ni declara el MVP completo.
...
- Contrato: `5.5`, `RF-001`, `RF-002`, `RF-003`, `RF-004`, `RF-005`, `RF-014`, `RF-029`, `CT-001`, `CT-002`, `CT-007`.
...
- [PZ-004A](FICHA-PZ-004A.md) aceptada expresamente.

Disponibilidad verificada de PZ-004A y autorizacion de datos/gasto para la prueba real. La memoria persistente se integra en PZ-010C.
...
|---|---|---|
| Crear | `app/chief_of_staff.py` | Solo esta pieza; detenerse si ya existe |
| Crear | `app/hq_runtime.py` | Solo esta pieza; detenerse si ya existe |
| Crear | `tests/test_chief_of_staff.py` | Solo esta pieza; detenerse si ya existe |
...
Entrada: Mision humana, decisiones, limites y gateway de PZ-004A; especialistas registrados progresivamente.

Salida: Chief real de aclaracion, propuesta de plan y consolidacion; punto de composicion para el resto del runtime.
...
1. Tres ciclos de aclaracion por version; no aceptar supuestos por el humano ni cambiar categorias de evidencia.
2. Versionar brief/plan y detectar cambio de alcance; plan nuevo siempre vuelve a puerta humana.
3. Contexto minimo y resumen de decisiones, nunca historial/CoT completo.
4. Separar propuestas del Chief de transiciones deterministas del motor; Chief no autoriza especialistas ni VBP.
5. Registrar especialistas mediante composicion explicita; si uno falta, bloquear. No usar simulated_specialists como reemplazo silencioso.
6. Proveer interfaces de repositorio, memoria, evidencia y aprobaciones; adaptadores reales se incorporan en sus piezas, sin declarar esas capacidades completas.

## 7. Criterios de aceptacion y pruebas obligatorias

- [ ] AC-01: Mision completa/ambigua/contradictoria; al cuarto ciclo solicitado se detiene.
- [ ] AC-02: Modificar brief invalida aprobacion exacta y conserva version previa.
- [ ] AC-03: Invocacion del Chief no incluye contexto ajeno, secretos ni expectativas evaluadoras.
- [ ] AC-04: Sin especialista requerido devolver bloqueo; salida del modelo no puede forzar estado autorizado.
- [ ] AC-05: Prueba real opt-in del Chief y pruebas offline separadas; todas las salidas en espanol.
...
- `python -B -m unittest discover -s tests -p test_chief_of_staff.py -v`
...
```text
Actua como constructor inicial exclusivamente de PZ-004B: Chief of Staff y composicion del runtime.
Trabaja en OminAIHQ y lee completa PIEZAS-PENDIENTES/FICHA-PZ-004B.md, sus fuentes y PIEZAS-PENDIENTES/00-REGLAS-Y-DECISIONES.md.
...
Comprueba aceptacion de PZ-004A y todos los requisitos del apartado 3. No uses archivos futuros ausentes como si ya existieran.

Puedes crear exclusivamente: app/chief_of_staff.py; app/hq_runtime.py; tests/test_chief_of_staff.py.
...
Archivos autorizados efectivamente - crear: app/chief_of_staff.py; app/hq_runtime.py; tests/test_chief_of_staff.py
