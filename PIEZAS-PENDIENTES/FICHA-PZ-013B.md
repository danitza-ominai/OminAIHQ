# PZ-013B - Interfaz de mision, trabajo y decisiones
...
Interfaz propia en espanol: perfil, mision, plan, seguimiento, evidencia, decisiones y memoria.

Problema que resuelve: interfaz de mision, trabajo y decisiones como tramo acotado del recorrido del contrato; no amplifica la iniciativa cliente ni declara el MVP completo.
...
- Contrato: `9.0-9.6`, `RF-019`, `RNF-010`, `RNF-011`, `CT-014`, `CT-016`.
...
- [PZ-013A](FICHA-PZ-013A.md) aceptada expresamente.

La base ligera HTML/CSS/JS queda aprobada. Concretar wireframe minimo y navegador de prueba; no crear un rediseño ajeno al alcance.
...
|---|---|---|
| Crear | `web/index.html` | Solo esta pieza; detenerse si ya existe |
| Crear | `web/app.js` | Solo esta pieza; detenerse si ya existe |
| Crear | `web/styles.css` | Solo esta pieza; detenerse si ya existe |
| Crear | `tests/test_ui_contracts.py` | Solo esta pieza; detenerse si ya existe |
...
Entrada: API local aceptada, contratos y etiquetas de estado/autoridad.

Salida: Interfaz propia en espanol: perfil, mision, plan, seguimiento, evidencia, decisiones y memoria.
...
1. Propuesta ligera HTML/CSS/JS sin build chain; aprobar esta eleccion antes de construir.
2. Pantallas muestran estado, tareas, intentos, limites, evidencia, errores y siguiente accion humana.
3. Vistas de revision completas; botones no sustituyen verificacion backend; evitar doble envio.
4. Renderizar contenido no confiable sin ejecutar HTML/scripts/ANSI; enlaces seguros y texto de fuente separado de instrucciones.
5. Teclado, foco visible, labels, contraste y anuncios de espera/error; no depender solo del color.
6. Memoria visible/corregible/eliminable con controles humanos; acciones irreversibles muestran alcance antes de confirmar.

## 7. Criterios de aceptacion y pruebas obligatorias

- [ ] AC-01: Recorrido visual real local desde perfil a revision, incluidos espera/error/pendiente.
- [ ] AC-02: Teclado y foco en formularios/dialogos; comprobacion manual de accesibilidad esencial.
- [ ] AC-03: XSS en mision/fuente/VBP no ejecuta; enlaces inseguros bloqueados.
- [ ] AC-04: UI oculta/inhabilita acciones segun estado, backend rechaza aun manipulando cliente.
- [ ] AC-05: Ninguna pantalla anuncia agentes reales o persistencia si runtime devuelve simulacion/fallo.
...
Perfil de efectos: **SERVIDOR_LOCAL_Y_PRUEBAS_VISUALES_AUTORIZADAS**.
...
```text
Actua como constructor inicial exclusivamente de PZ-013B: Interfaz de mision, trabajo y decisiones.
Trabaja en OminAIHQ y lee completa PIEZAS-PENDIENTES/FICHA-PZ-013B.md, sus fuentes y PIEZAS-PENDIENTES/00-REGLAS-Y-DECISIONES.md.
...
Comprueba aceptacion de PZ-013A y todos los requisitos del apartado 3. No uses archivos futuros ausentes como si ya existieran.

Puedes crear exclusivamente: web/index.html; web/app.js; web/styles.css; tests/test_ui_contracts.py.
Puedes modificar exclusivamente: app/http_api.py.
...
Respeta el perfil de efectos SERVIDOR_LOCAL_Y_PRUEBAS_VISUALES_AUTORIZADAS; todo permiso de instalacion, datos, nube, gasto o publicacion debe estar aprobado de forma concreta.
...
Archivos autorizados efectivamente - crear: web/index.html; web/app.js; web/styles.css; tests/test_ui_contracts.py
Archivos autorizados efectivamente - modificar: app/http_api.py
