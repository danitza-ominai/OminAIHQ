# PZ-009C - Imagenes y expediente saneado de competencia
...
Interpretacion de imagen referenciada al original y expediente publico autorizado separado del privado.

Problema que resuelve: imagenes y expediente saneado de competencia como tramo acotado del recorrido del contrato; no amplifica la iniciativa cliente ni declara el MVP completo.
...
- Contrato: `11.7`, `11.9`, `PT-003`, `PT-008`, `CT-005`, `CT-015`.
...
- [PZ-009B](FICHA-PZ-009B.md) aceptada expresamente.

Aprobar limites de pixeles, soporte multimodal real y autorizacion concreta de las copias de competencia.
...
|---|---|---|
| Crear | `app/image_intake.py` | Solo esta pieza; detenerse si ya existe |
| Crear | `app/sanitized_dossier.py` | Solo esta pieza; detenerse si ya existe |
| Crear | `tests/test_image_intake.py` | Solo esta pieza; detenerse si ya existe |
| Crear | `tests/test_sanitized_dossier.py` | Solo esta pieza; detenerse si ya existe |
...
Entrada: PNG/JPG/JPEG y fuentes cuya copia saneada autorice el usuario humano.

Salida: Interpretacion de imagen referenciada al original y expediente publico autorizado separado del privado.
...
1. Aplicar mismos limites globales del lote; no reiniciar contadores de archivos al cambiar formato.
2. Limitar dimensiones/pixeles descomprimidos y validar tipo; fijar umbral tecnico antes de iniciar construccion.
3. Separar datos observados, interpretacion e incertidumbre; referencias a imagen/region cuando sea posible.
4. Expediente publico incluye solo copias autorizadas, manifest de saneamiento y procedencia sin revelar rutas privadas ni metadatos personales.
5. Enviar al modelo solo imagen/copia saneada aprobada; no subir originales privados por defecto ni publicar el expediente.

## 7. Criterios de aceptacion y pruebas obligatorias

- [ ] AC-01: Imagen valida, corrupta, bomba de descompresion y extension falsa.
- [ ] AC-02: Incertidumbre visual no se convierte en hecho confirmado.
- [ ] AC-03: Datos EXIF/metadatos sensibles y secretos no aparecen en copia publica.
- [ ] AC-04: Original y copia no se confunden; revocar permiso impide exportacion publica futura.
- [ ] AC-05: Prueba multimodal real opt-in con imagen saneada y costo autorizado.
...
- `python -B -m unittest discover -s tests -p test_image_intake.py -v`
- `python -B -m unittest discover -s tests -p test_sanitized_dossier.py -v`
...
Perfil de efectos: **IO_LOCAL_Y_MODELO_SOLO_CON_DATOS_SANEADOS_AUTORIZADOS**.
...
```text
Actua como constructor inicial exclusivamente de PZ-009C: Imagenes y expediente saneado de competencia.
Trabaja en OminAIHQ y lee completa PIEZAS-PENDIENTES/FICHA-PZ-009C.md, sus fuentes y PIEZAS-PENDIENTES/00-REGLAS-Y-DECISIONES.md.
...
Comprueba aceptacion de PZ-009B y todos los requisitos del apartado 3. No uses archivos futuros ausentes como si ya existieran.

Puedes crear exclusivamente: app/image_intake.py; app/sanitized_dossier.py; tests/test_image_intake.py; tests/test_sanitized_dossier.py.
...
Respeta el perfil de efectos IO_LOCAL_Y_MODELO_SOLO_CON_DATOS_SANEADOS_AUTORIZADOS; todo permiso de instalacion, datos, nube, gasto o publicacion debe estar aprobado de forma concreta.
...
Archivos autorizados efectivamente - crear: app/image_intake.py; app/sanitized_dossier.py; tests/test_image_intake.py; tests/test_sanitized_dossier.py
