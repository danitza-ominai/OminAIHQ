# PZ-005A - Research y consulta gobernada de fuentes
...
Agente real de investigacion con hallazgos atomicos y recorrido verificable claim-fuente.

Problema que resuelve: research y consulta gobernada de fuentes como tramo acotado del recorrido del contrato; no amplifica la iniciativa cliente ni declara el MVP completo.
...
- Contrato: `5.6`, `6.3`, `11.7`, `RF-010`, `RF-011`, `CT-006`, `DN-003`.
...
- [PZ-004B](FICHA-PZ-004B.md) aceptada expresamente.

Elegir fuentes reales saneadas, dominios y limites de lectura antes de iniciar construccion. Si hacen falta parsers adicionales, esperan PZ-009B/C.
...
|---|---|---|
| Crear | `app/research_analyst.py` | Solo esta pieza; detenerse si ya existe |
| Crear | `app/source_reader.py` | Solo esta pieza; detenerse si ya existe |
| Crear | `tests/test_research_analyst.py` | Solo esta pieza; detenerse si ya existe |
...
Entrada: Pregunta y lista de fuentes autorizadas en el plan; extractos aportados y URLs publicas permitidas.

Salida: Agente real de investigacion con hallazgos atomicos y recorrido verificable claim-fuente.
...
1. Primero fuentes aportadas; web de solo lectura solo si el plan la autoriza. Tool parametrizada con allowlist, timeout y tamano finitos.
2. Bloquear rutas locales arbitrarias, URLs internas/loopback/metadatos y redirecciones fuera del permiso para evitar SSRF; no consultas empresariales libres.
3. Conservar localizador, fechas distintas de publicacion/consulta, extracto y limitaciones; no inventar cita.
4. Confianza alta solo con primaria solida o dos independientes; contradicciones y obsolescencia quedan visibles.
5. Registrar en hq_runtime exclusivamente este especialista y su tool; no ejecutar iniciativas del VBP.

## 7. Criterios de aceptacion y pruebas obligatorias

- [ ] AC-01: Fuente real autorizada y referencia inspeccionable; pruebas offline de inaccesible, falsa, conflictiva y antigua.
- [ ] AC-02: URL/ruta no autorizada bloqueada antes de IO; redireccion y exceso de bytes/tiempo rechazados.
- [ ] AC-03: Un hallazgo sin fuente se etiqueta no verificado; el critico bloquea aprobacion ordinaria.
- [ ] AC-04: Intento de prompt injection en fuente no cambia permisos ni extrae secretos.
- [ ] AC-05: Invocacion real opt-in separada de simulaciones; matriz claim/fuente y herramientas usadas.
...
- `python -B -m unittest discover -s tests -p test_research_analyst.py -v`
...
Perfil de efectos: **LECTURA_EXTERNA_AUTORIZADA_Y_PRUEBAS_OFFLINE**.
...
```text
Actua como constructor inicial exclusivamente de PZ-005A: Research y consulta gobernada de fuentes.
Trabaja en OminAIHQ y lee completa PIEZAS-PENDIENTES/FICHA-PZ-005A.md, sus fuentes y PIEZAS-PENDIENTES/00-REGLAS-Y-DECISIONES.md.
...
Comprueba aceptacion de PZ-004B y todos los requisitos del apartado 3. No uses archivos futuros ausentes como si ya existieran.

Puedes crear exclusivamente: app/research_analyst.py; app/source_reader.py; tests/test_research_analyst.py.
...
Respeta el perfil de efectos LECTURA_EXTERNA_AUTORIZADA_Y_PRUEBAS_OFFLINE; todo permiso de instalacion, datos, nube, gasto o publicacion debe estar aprobado de forma concreta.
...
Archivos autorizados efectivamente - crear: app/research_analyst.py; app/source_reader.py; tests/test_research_analyst.py
