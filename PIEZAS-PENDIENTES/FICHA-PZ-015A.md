# PZ-015A - Pruebas integrales y gate del MVP
...
Informe de cierre por RF/RNF/CT/control del checklist y gates; sin correccion de producto.

Problema que resuelve: pruebas integrales y gate del mvp como tramo acotado del recorrido del contrato; no amplifica la iniciativa cliente ni declara el MVP completo.
...
- Contrato: `RF-001`, `RF-030`, `RNF-001`, `RNF-015`, `CT-001`, `CT-017`, `15.7`, `C.18`.
...
- [PZ-014B](FICHA-PZ-014B.md) aceptada expresamente.

El gate final se ejecuta solo con autorizacion del usuario/evaluador y costo restante conocido; no publica ni presenta el proyecto.
...
|---|---|---|
| Crear | `tests/test_hq_end_to_end.py` | Solo esta pieza; detenerse si ya existe |
| Crear | `tests/test_release_gates.py` | Solo esta pieza; detenerse si ya existe |
| Crear | `evaluation/release_protocol.md` | Solo esta pieza; detenerse si ya existe |
...
Entrada: Version desplegada/commit exacto, matriz completa, dataset aprobado y evidencia real.

Salida: Informe de cierre por RF/RNF/CT/control del checklist y gates; sin correccion de producto.
...
1. Evaluar todos los controles aplicables, no solo happy path; cada evidencia tiene ID/version/fecha/metodo.
2. Gate exige runtime real, fuente real, persistencia/reinicio, memoria y decisiones humanas reales.
3. Demo objetivo 150 segundos en mision preparada; limite general 1200 no sustituye objetivo de demo.
4. Holdout ejecutado por evaluador aislado despues de congelar version; no reutilizar para optimizar.
5. No aprobar un P0 ausente por promedio de P1 ni declarar N/A sin justificacion y aprobacion humana.
6. Defectos se reportan; correcciones requieren encargo separado, no editar modulos bajo pretexto de tests.

## 7. Criterios de aceptacion y pruebas obligatorias

- [ ] AC-01: Recorrido completo local y nube con dos puertas, evidencia, descarga y auditoria.
- [ ] AC-02: Interrupcion/reinicio, memoria normal y conflictiva, original desaparecido y accion prohibida.
- [ ] AC-03: 82 regresiones iniciales y todas las agregadas sin suprimir pruebas; distinguir unitarias, subcasos y reales.
- [ ] AC-04: Gate C.18 y 15.7 con evidencia o rechazo explicito, cero casillas completadas por existir una ficha.
- [ ] AC-05: Prueba manual accesible y medicion real del caso de demo; falta de live/holdout impide declarar LISTA_PARA_DEMO.
...
- `python -B -m unittest discover -s tests -p test_hq_end_to_end.py -v`
- `python -B -m unittest discover -s tests -p test_release_gates.py -v`
...
Perfil de efectos: **PRUEBAS_REALES_LOCAL_NUBE_CON_PRESUPUESTO_APROBADO**.
...
```text
Actua como constructor inicial exclusivamente de PZ-015A: Pruebas integrales y gate del MVP.
Trabaja en OminAIHQ y lee completa PIEZAS-PENDIENTES/FICHA-PZ-015A.md, sus fuentes y PIEZAS-PENDIENTES/00-REGLAS-Y-DECISIONES.md.
...
Comprueba aceptacion de PZ-014B y todos los requisitos del apartado 3. No uses archivos futuros ausentes como si ya existieran.

Puedes crear exclusivamente: tests/test_hq_end_to_end.py; tests/test_release_gates.py; evaluation/release_protocol.md.
...
Respeta el perfil de efectos PRUEBAS_REALES_LOCAL_NUBE_CON_PRESUPUESTO_APROBADO; todo permiso de instalacion, datos, nube, gasto o publicacion debe estar aprobado de forma concreta.
...
Archivos autorizados efectivamente - crear: tests/test_hq_end_to_end.py; tests/test_release_gates.py; evaluation/release_protocol.md
