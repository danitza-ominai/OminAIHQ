# PZ-009B - Archivos de texto, PDF y DOCX con originales privados
...
Ingesta limitada de TXT/MD/PDF/DOCX, original local verificable y extraccion estructurada.

Problema que resuelve: archivos de texto, pdf y docx con originales privados como tramo acotado del recorrido del contrato; no amplifica la iniciativa cliente ni declara el MVP completo.
...
- Contrato: `11.7`, `PT-003`, `CT-005`, `CT-015`, `RNF-004`.
...
- [PZ-009A](FICHA-PZ-009A.md) aceptada expresamente.

Aprobar librerias/versiones y raiz local de datos; los fixtures binarios de prueba se generan en area temporal autorizada, no se agregan por fuera de la lista de fuentes.
...
|---|---|---|
| Crear | `app/file_intake.py` | Solo esta pieza; detenerse si ya existe |
| Crear | `app/document_extractors.py` | Solo esta pieza; detenerse si ya existe |
| Crear | `tests/test_file_intake.py` | Solo esta pieza; detenerse si ya existe |
...
Entrada: Archivos aportados expresamente a una mision y raiz local de artefactos autorizada.

Salida: Ingesta limitada de TXT/MD/PDF/DOCX, original local verificable y extraccion estructurada.
...
1. Cinco archivos por mision; 20 MB por archivo, 50 MB total y 10 enlaces; validar antes de aceptar el lote completo.
2. Comprobar extension/tipo real, rutas seguras, nombres colisionados y archivos corruptos/cifrados; no ejecutar macros ni contenido.
3. Bloquear secretos y requerir copia saneada; confidencialidad requiere advertencia y confirmacion humana antes de procesamiento.
4. Guardar original local con hash, tamano, propietario, mision/version; cada extracto conserva pagina/seccion.
5. PDF escaneado sin soporte OCR explicito se marca sin extraccion, no se inventa texto; soporte visual queda en PZ-009C.
6. Dependencias de parsing solo con lista/version aprobadas; sin enviar originales a servicios externos.

## 7. Criterios de aceptacion y pruebas obligatorias

- [ ] AC-01: Limites exactos y excedidos, extension falsa, path traversal, archivo corrupto/cifrado y duplicado.
- [ ] AC-02: Extraccion de cada formato con localizador real; original y hash verificables.
- [ ] AC-03: Secretos bloqueados y datos confidenciales no procesados sin confirmacion.
- [ ] AC-04: Fallo durante guardado no deja registro afirmando que original existe.
- [ ] AC-05: Pruebas de IO solo en directorio temporal aislado autorizado; no datos privados reales en repositorio.
...
- `python -B -m unittest discover -s tests -p test_file_intake.py -v`
...
Perfil de efectos: **IO_LOCAL_DE_PRUEBA_Y_ORIGINALES_AUTORIZADOS**.
...
```text
Actua como constructor inicial exclusivamente de PZ-009B: Archivos de texto, PDF y DOCX con originales privados.
Trabaja en OminAIHQ y lee completa PIEZAS-PENDIENTES/FICHA-PZ-009B.md, sus fuentes y PIEZAS-PENDIENTES/00-REGLAS-Y-DECISIONES.md.
...
Comprueba aceptacion de PZ-009A y todos los requisitos del apartado 3. No uses archivos futuros ausentes como si ya existieran.

Puedes crear exclusivamente: app/file_intake.py; app/document_extractors.py; tests/test_file_intake.py.
Puedes modificar exclusivamente: pyproject.toml; app/hq_runtime.py.
...
Respeta el perfil de efectos IO_LOCAL_DE_PRUEBA_Y_ORIGINALES_AUTORIZADOS; todo permiso de instalacion, datos, nube, gasto o publicacion debe estar aprobado de forma concreta.
...
Archivos autorizados efectivamente - crear: app/file_intake.py; app/document_extractors.py; tests/test_file_intake.py
Archivos autorizados efectivamente - modificar: pyproject.toml; app/hq_runtime.py
