# PZ-010D - Archivo, eliminacion y ciclo de datos
...
Operaciones distintas de archivar y eliminar, con confirmacion y efectos de evidencia.

Problema que resuelve: archivo, eliminacion y ciclo de datos como tramo acotado del recorrido del contrato; no amplifica la iniciativa cliente ni declara el MVP completo.
...
- Contrato: `9.6`, `11.6-11.7`, `DN-004`, `CT-015`.
...
- [PZ-010C](FICHA-PZ-010C.md) aceptada expresamente.

Aprobar politica exacta de purga/retencion y raiz de pruebas. El archivo de una mision activa debe suspender su ejecucion antes de liberar el cupo.
...
|---|---|---|
| Crear | `app/data_lifecycle.py` | Solo esta pieza; detenerse si ya existe |
| Crear | `tests/test_data_lifecycle.py` | Solo esta pieza; detenerse si ya existe |
...
Entrada: Misiones, memorias y artefactos locales; solicitudes humanas identificadas.

Salida: Operaciones distintas de archivar y eliminar, con confirmacion y efectos de evidencia.
...
1. Cancelar conserva historia; archivar oculta sin borrar; eliminar exige confirmacion especifica con objetivo/alcance/version.
2. Antes de borrar resolver ruta canonica, pertenencia y limites; nunca directorios amplios, symlinks externos o rutas aportadas por modelo.
3. Original desaparecido preaprobacion: parar y mostrar EVIDENCIA_NO_DISPONIBLE; restaurar/reemplazar/retirar requiere decision humana.
4. Postaprobacion mantener VBP y decision historica; evidencia ELIMINADA_POR_EL_USUARIO y paquete no completamente verificable.
5. Memoria borrada no queda en versiones recuperables/logs/exportaciones nuevas; minimizacion compatible con historial VBP ya aprobado.
6. No realizar eliminacion real de documentos del usuario durante construccion; solo fixtures autorizadas.

## 7. Criterios de aceptacion y pruebas obligatorias

- [ ] AC-01: Archivar no elimina ni autoriza silenciosamente otra ejecucion en curso.
- [ ] AC-02: Eliminar sin confirmacion, por actor ajeno o path traversal se rechaza.
- [ ] AC-03: Pre y postaprobacion producen efectos distintos, sin falsear verificabilidad.
- [ ] AC-04: Confirmacion repetida idempotente; cambio de version obliga nueva confirmacion.
- [ ] AC-05: Buscar contenido sintetico borrado en todas las tablas/logs del sandbox; comprobar purga prevista.
...
- `python -B -m unittest discover -s tests -p test_data_lifecycle.py -v`
...
Perfil de efectos: **ELIMINACION_SOLO_DE_DATOS_DE_PRUEBA_EXPLICITAMENTE_AUTORIZADOS**.
...
```text
Actua como constructor inicial exclusivamente de PZ-010D: Archivo, eliminacion y ciclo de datos.
Trabaja en OminAIHQ y lee completa PIEZAS-PENDIENTES/FICHA-PZ-010D.md, sus fuentes y PIEZAS-PENDIENTES/00-REGLAS-Y-DECISIONES.md.
...
Comprueba aceptacion de PZ-010C y todos los requisitos del apartado 3. No uses archivos futuros ausentes como si ya existieran.

Puedes crear exclusivamente: app/data_lifecycle.py; tests/test_data_lifecycle.py.
...
Respeta el perfil de efectos ELIMINACION_SOLO_DE_DATOS_DE_PRUEBA_EXPLICITAMENTE_AUTORIZADOS; todo permiso de instalacion, datos, nube, gasto o publicacion debe estar aprobado de forma concreta.
...
Archivos autorizados efectivamente - crear: app/data_lifecycle.py; tests/test_data_lifecycle.py
