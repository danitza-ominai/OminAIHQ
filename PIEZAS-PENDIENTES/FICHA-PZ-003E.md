# PZ-003E - VBP canonico y validacion determinista SIMULADA
...
Borrador unico Markdown con manifest, 18 secciones, huella y reporte determinista; no aprobacion.

Problema que resuelve: vbp canonico y validacion determinista simulada como tramo acotado del recorrido del contrato; no amplifica la iniciativa cliente ni declara el MVP completo.
...
- Contrato: `6`, `11.8`, `RF-014`, `RF-018`, `RF-028`, `RF-030`, `DN-005`, `CT-010`.
...
- [PZ-003D](FICHA-PZ-003D.md) aceptada expresamente.

Las reglas de calculo y huella deben quedar explicitas y aprobadas; la puntuacion de una simulacion no acredita calidad del producto real.
...
|---|---|---|
| Crear | `app/vbp_document.py` | Solo esta pieza; detenerse si ya existe |
| Crear | `app/vbp_validation.py` | Solo esta pieza; detenerse si ya existe |
| Crear | `tests/test_vbp_document.py` | Solo esta pieza; detenerse si ya existe |
...
Entrada: Salidas etiquetadas y referencias del motor SIMULADA.

Salida: Borrador unico Markdown con manifest, 18 secciones, huella y reporte determinista; no aprobacion.
...
1. Construir por funciones deterministas: ordenar sin inventar contenido ni omitir limitaciones.
2. Separar hechos respaldados, supuestos, propuestas, decisiones y pendientes; conservar localizadores.
3. Aplicar bloqueadores antes del puntaje; pesos 30/25/20/15/10 y umbrales de 11.8; no crear un juez autoaprobador.
4. Las fuentes ficticias siguen marcadas SIMULADA y no satisfacen evidencia real para competencia.
5. Congelar contenido candidato y dominio de huella antes de decision; registros de integridad no pueden causar huellas autorreferentes.

## 7. Criterios de aceptacion y pruebas obligatorias

- [ ] AC-01: Contar e identificar las 18 secciones y manifest obligatorio.
- [ ] AC-02: Rechazar seccion ausente, cita inexistente, fuente contradictoria oculta y metrica inventada.
- [ ] AC-03: Comprobar fronteras 69/70/79/80 y un caso con puntaje alto pero bloqueador.
- [ ] AC-04: Alterar un dato material cambia huella; renderizar sin cambios preserva contenido.
- [ ] AC-05: Sin aprobacion no hay estado FINALIZADA ni descarga marcada como final.
...
- `python -B -m unittest discover -s tests -p test_vbp_document.py -v`
...
```text
Actua como constructor inicial exclusivamente de PZ-003E: VBP canonico y validacion determinista SIMULADA.
Trabaja en OminAIHQ y lee completa PIEZAS-PENDIENTES/FICHA-PZ-003E.md, sus fuentes y PIEZAS-PENDIENTES/00-REGLAS-Y-DECISIONES.md.
...
Comprueba aceptacion de PZ-003D y todos los requisitos del apartado 3. No uses archivos futuros ausentes como si ya existieran.

Puedes crear exclusivamente: app/vbp_document.py; app/vbp_validation.py; tests/test_vbp_document.py.
...
Archivos autorizados efectivamente - crear: app/vbp_document.py; app/vbp_validation.py; tests/test_vbp_document.py
